...
void adventurerEffect(int drawntreasure, struct gameState *state, int currentPlayer, int cardDrawn, int temphand[], int z) { //bug introduced, reveals cards until 3 treasure cards instead of 2
  while(drawntreasure<3){ //change to 3 for bug
...
void smithyEffect(struct gameState *state, int currentPlayer, int handPos) {//bug, draw two cards instead of 3
			int i;
	      //+3 Cards, changed to 2 for bug
      for (i = 0; i < 2; i++)
	{
	  drawCard(currentPlayer, state);
	}
...
void villageEffect(struct gameState *state, int currentPlayer, int handPos){//bug, no drawn card, and +3 actions
	      //+1 Card
      //drawCard(currentPlayer, state);
			//removed for bug
...
void salvagerEffect(struct gameState *state, int currentPlayer, int handPos, int choice1){ //discards 2 cards instead of one
	      //+1 buy
      state->numBuys++;
      ...
      if (choice1)
	{
      ...
      //discard card, extra added for bug
      discardCard(handPos, currentPlayer, state, 0);
      discardCard(handPos, currentPlayer, state, 0);
}
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    ...
    case cutpurse:
      cutpurseEffect(state, currentPlayer);
      return 0;
    ...
    case salvager:
      salvagerEffect(state, currentPlayer, handPos, choice1);
      return 0;
